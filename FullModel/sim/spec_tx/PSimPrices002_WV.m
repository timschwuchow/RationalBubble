...
%  PSimPrices002_WV.m		-	Solve VF and simulate (parallel)
%  Working Version
...
StateSpecMon(:,1)		=	smap(ceil(epts/2),ceil(upts/2),ceil(dpts/2));
...
StateSpecComp(:,1)	=	smap(ceil(epts/2),ceil(upts/2),ceil(dpts/2));
...
%  		Eta(:,t) 				=	Mu(:,t);
		SpecInvComp(:,t)			=	max(N*(normcdf( -(x+b*norminv(1-H/N,0,bmvargrid))/(1-b) - emap(smapinv(StateSpecComp(:,t),1))'/(1-b*regrid) - ce1grid*umap(smapinv(StateSpecComp(:,t),2))',0,bmvargrid)-1)+H*thresh,0);
...
save(['outmat/pricesim' int2str(repng) '.mat'],'PriceSpecMon','PriceSpecComp','PriceCOnly','PriceEff','SpecInvMon','StateSpecMon','StateCons', '-V7.3');
