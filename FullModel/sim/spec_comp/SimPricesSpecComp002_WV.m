...
%  SimPricesSpecComp002_WV.m		-	Simulate Prices for competitive speculative market
%  Working Version
...
for i=1:1
	...
	for t=1:T
		if t > 1
			...
			DeltaSpec(:,t)			=	cd2*DeltaSpec(:,t-1) + ce2*Mu(:,t-1) + cb2*(norminv(1-H/N,0,bmvar) - norminv(1-(H-SpecInv(:,t-1))/N,0,bmvar));
...
%  			SpecInv(:,t)			=	max(N*(normcdf( -L/b/(1-L)*x -L/(1-L)*norminv(1-H/N,0,bmvar) - L/b/(1-L*re)*Eta(:,t) + L/b*(b*cd1*ce2-ce1)*Mu(:,t) + L/b/(1-cd2*L)*cd1*(b*cd2 - 1)*(DeltaSpec(:,t)+ce2/cd2*Mu(:,t)+cb2/cd2*norminv(1-H/N,0,bmvar)),0,bmvar)-1)+H*0.99,0);
			SpecInv(:,t)			=	max(N*(normcdf( -(x+b*norminv(1-H/N,0,bmvar))/(1-b) - Eta(:,t)/(1-b*re) - cd1*DeltaSpec(:,t) - ce1*Mu(:,t),0,bmvar)-1)+H*thresh,0);
			Score(:,t)				=	-x/(1-b) -b*norminv(1-H/N,0,bmvar)/(1-b) - Eta(:,t)/(1-b*re) - ce1*Mu(:,t) - cd1*DeltaSpec(:,t);
%  			Score2(:,t)				=	-L/b/(1-L)*x -L/(1-L)*norminv(1-H/N,0,bmvar) - L/b/(1-L*re)*Eta(:,t) + L/b*(b*cd1*ce2-ce1)*Mu(:,t) + L/b/(1-cd2*L)*cd1*(b*cd2 - 1)*(DeltaSpec(:,t)+ce2/cd2*Mu(:,t));
		else
			Eta(:,t) 				=	Mu(:,t);
...
%  			SpecInv(:,t)			=	max(N*(normcdf( -L/b/(1-L)*x -L/(1-L)*norminv(1-H/N,0,bmvar) - L/b/(1-L*re)*Eta(:,t) + L/b*(b*cd1*ce2-ce1)*Mu(:,t) + L/b/(1-cd2*L)*cd1*(b*cd2 - 1)*(ce2/cd2*Mu(:,t)+cb2/cd2*norminv(1-H/N,0,bmvar)),0,bmvar)-1)+H*0.99,0);
			SpecInv(:,t)			=	max(N*(normcdf( -(x+b*norminv(1-H/N,0,bmvar))/(1-b) - Eta(:,t)/(1-b*re) - ce1*Mu(:,t),0,bmvar)-1)+H*thresh,0);
			Score(:,t)				=	-x/(1-b) -b*norminv(1-H/N,0,bmvar)/(1-b) - Eta(:,t)/(1-b*re) -ce1*Mu(:,t);
%  			Score2(:,t)				=	-L/b/(1-L)*x -L/(1-L)*norminv(1-H/N,0,bmvar) - L/b/(1-L*re)*Eta(:,t) + L/b*(b*cd1*ce2-ce1)*Mu(:,t) + L/b/(1-cd2*L)*cd1*(b*cd2 - 1)*(ce2/cd2*Mu(:,t));
		end
	...
	PriceInd							=	(SpecInv(:,2:T)>0).*(SpecInv(:,2:T)<thresh*H).*(SpecInv(:,1:T-1)>0).*(SpecInv(:,1:T-1)<thresh*H);
	PriceInd2						=	(SpecInv>0).*(SpecInv<thresh*H);
	[TVec(PriceInd2(1,:)==1)',  Eta(1,PriceInd2(1,:)==1)', -Eta(1,PriceInd2(1,:)==1)'/(1-b*re),Mu(1,PriceInd2(1,:)==1)', -Mu(1,PriceInd2(1,:)==1)'*ce1,DeltaSpec(1,PriceInd2(1,:)==1)',-DeltaSpec(1,PriceInd2(1,:)==1)'*cd1, Score(1,PriceInd2(1,:)==1)', SpecInv(1,PriceInd2(1,:)==1)', PriceSpecComp(1,PriceInd2(1,:)==1)']
	fprintf('Mean all prices %5.4f \nMean interior price %5.4f\n',mean(mean(PriceSpecComp)),mean(mean(PriceSpecComp(PriceInd2==1))))
