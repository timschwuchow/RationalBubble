...
function [imap emap umap dmap smap smapinv nstate]	=	statemappar002(pgrid,vdgrid,cb2grid,cd2grid,bmvargrid,ipts,epts,upts,dpts,maxshare,esd,usd,dsd,H,N)
...
for i=1:numel(regrid)
	maxbias		=	0.5*cb2grid(i)*(norminv(1-H/N,0,bmvargrid(i)) - norminv(1-(H-imap(ipts))/N,0,bmvargrid(i)))/(1-cd2grid(i));
